...
def plot_barplot(metrics_df, save_path=None):
    ...
    ax.bar(x, metrics_df["iou_mean_mean"], bar_width, yerr=metrics_df["iou_mean_std"], label="IoU")
    ax.bar(x + bar_width, metrics_df["dice_mean_mean"], bar_width, yerr=metrics_df["dice_mean_std"], label="Dice")
    ax.bar(x + bar_width*2, metrics_df["precision_mean"], bar_width, yerr=metrics_df["precision_std"], label="Precision")
    ax.bar(x + bar_width*3, metrics_df["recall_mean"], bar_width, yerr=metrics_df["recall_std"], label="Recall")
    ax.bar(x + bar_width*4, metrics_df["f1_score_mean"], bar_width, yerr=metrics_df["f1_score_std"], label="F1")
...
def plot_error_types(gt_mask, pred_mask, metrics, name, save=False, save_path=None, suptitle=False, legend=False):

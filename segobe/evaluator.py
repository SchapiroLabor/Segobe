...
class SegmentationEvaluator:
    ...
    def get_metric_matrix(self):
        ...
        for i, gt_id in enumerate(self.gt_ids):
            ...
            for j, pred_id in enumerate(self.pred_ids):
                ...
                dice = (2 * intersection) / (gt_size + pred_size) if (gt_size + pred_size) > 0 else 0
                moc = (intersection / gt_size + intersection / pred_size) / 2 if gt_size > 0 and pred_size > 0 else 0
                iou_matrix[i, j] = iou
    ...
    def construct_cost_matrix(self, metric_matrix):
        ...
        cost_matrix[:self.n_gt, :self.n_pred] = 1 - metric_matrix     # A: Top-left block = real costs
        cost_matrix[self.n_gt:, self.n_pred:] = (1 - metric_matrix).T     # D: Bottom-right = transpose of A
        top_right = self.unmatched_cost * np.eye(self.n_gt) + (1 - np.eye(self.n_gt))
        cost_matrix[:self.n_gt, self.n_pred:] = top_right     # B: Top-right = unmatched GTs
        bottom_left = self.unmatched_cost * np.eye(self.n_pred) + (1 - np.eye(self.n_pred))
    ...
    def evaluate(self):
        ...
        self.iou_graph.remove_nodes_from([
            n for n in self.iou_graph.nodes
            if (n.startswith("pred_") and int(n.split("_")[1]) in matched_pred) or
               (n.startswith("gt_") and int(n.split("_")[1]) in matched_gt)
        ])
        ...
        for component in nx.connected_components(self.iou_graph):
            ...
            if ng == 1 and np_ > 1:
                ...
                catastrophes += 1
                catastrophe_details.append({"gts": sorted(gt_nodes), "preds": sorted(pred_nodes)})
